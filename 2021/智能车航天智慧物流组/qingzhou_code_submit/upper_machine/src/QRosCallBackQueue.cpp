...
QRosCallBackQueue::QRosCallBackQueue() {
	...
	if (::socketpair(AF_UNIX, SOCK_STREAM, 0, QRosCallBackQueue::m_sigtermFd))
		qFatal("Couldn't create TERM socketpair");
	m_snTerm = new QSocketNotifier(QRosCallBackQueue::m_sigtermFd[1],
	...
	connect(this, &QRosCallBackQueue::onCallback, this,
	        &QRosCallBackQueue::processCallbacks, Qt::QueuedConnection);
}

QRosCallBackQueue::~QRosCallBackQueue() {}

void QRosCallBackQueue::addCallback(const ros::CallbackInterfacePtr &callback,
                                    uint64_t owner_id) {
...
void QRosCallBackQueue::termSignalHandler(int signal) {
	...
	// Only reentrant methods can be called from here ! eg. any call to new or
	// malloc could cause deadlocks

	// set flag to shutdown ros ; this is needed if for example a nodehandle is
	// waiting for the rosmaster
	ros::requestShutdown();

	// send something on the socket to notify Qt a shutdown is requested
	char receivedSignal = static_cast<char>(signal);
	(void)::write(QRosCallBackQueue::m_sigtermFd[0], &receivedSignal,
...
void QRosCallBackQueue::handleSigTerm() {
	// we received something on the socket : this means the user wants to quit
	// the application

	m_snTerm->setEnabled(false);
	char receivedSignal;
	(void)::read(QRosCallBackQueue::m_sigtermFd[1], &receivedSignal,
	             sizeof(receivedSignal));
	...
	qDebug() << "Received" << signalStr
	         << "signal, properly quitting Qt eventloop...";
