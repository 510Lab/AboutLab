...
{ 
   ...
   handle.param("ticksPer2PI",ticksPer2PI,ticksPer2PI);                                 //每圈脉冲数
    ...
    try{ //异常处理
      ...
      std::cout<<"[dzactuator-->]"<<"Unable to open port! "<<e.what()<<std::endl;                
    }
    if(ser.isOpen()){
...
void actuator::callback_move_base(const geometry_msgs::Twist::ConstPtr &msg) //对应cmd_vel话题，对应geometry_msgs/Twist消息
{
   ...
   if (v < 0.0) {
      ROS_WARN("Going backwards is not supported, vx = %f", v);
      v = 0.0;
   }
...
void actuator::run()
{
    ...
    while(ros::ok()){
		...
		point[0].x = -0.25;  point[0].y = -0.20;
		point[1].x = 0.25;   point[1].y = -0.20;
		point[2].x = 0.25;   point[2].y = 0.20;
		point[3].x = -0.25;  point[3].y = 0.20;
...
void actuator::recvCarInfoKernel()
{
    ...
    if(recvd_flag){                                                  //数据解析，接收到的数据转存
		...
		magX = (float)tempmagX*0.14*1e-6;                                 //磁力计处理
		magY = (float)tempmagY*0.14*1e-6;
		magZ = (float)tempmagZ*0.14*1e-6;
